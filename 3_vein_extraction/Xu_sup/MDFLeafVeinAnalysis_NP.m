function results = MDFLeafVeinAnalysis_NP(FolderName,MicronPerPixel,DownSample,threshold,ShowFigs,ExportFigs,FullLeaf,FullMetrics)
%% set up directories
...
sk_width = floor(7/DownSample);
E_width = 0.5+0.5/DownSample;
...
% need to revert bwCNNMask to within function
...
[bwCNN, sk, skLoop, skTree, skRing, skMask] = fnc_skeleton(imCNN,bwVein,bwMask,threshold,FullLeaf);
%% calculate the width
step = step+1;
...
if exist(mask_name,'file') == 2
    ...
    % only work with the largest area
    bw_mask = bwareafilt(bw_mask,1);
else
    disp('        No mask image')
    bw_mask = true(nY,nX);
end
...
function [bwCNN, skFinal, skLoop, skTree, skRing, skMask] = fnc_skeleton(im_in,bwVein,bwMask,threshold,FullLeaf)
%warning off
if islogical(im_in)
...
if any(skLoopInitial(:))
    ...
    DamageMask = imclose(DamageMask,[0 1 0; 1 1 1; 0 1 0]);
    ...
    % remove any single pixel spurs
    spur = bwmorph(skLoop,'endpoints');
    skLoop(spur) = 0;
    skMask(spur) = 0;
    % remove any left over single pixels from the mask
    skMask(skLoop) = 0;
    skMask = bwmorph(skMask,'clean');
    skMask = skMask | skLoop;
...
else % catch the case where there are no loops
    skFinal =  bwmorph(bwCNN,'thin',Inf);
    skFinal = bwmorph(skFinal,'thin',inf);
    skTree = skFinal;
    skLoop = false(size(skFinal));
    skRing = false(size(skFinal));
    skMask = bwMask;
end
end
