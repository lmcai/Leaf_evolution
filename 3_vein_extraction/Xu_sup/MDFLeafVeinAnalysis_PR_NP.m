function [results, PR_methods] = MDFLeafVeinAnalysis_PR_NP(FolderName,MicronPerPixel,DownSample,ShowFigs,ExportFigs)
%% set up directories
...
for iM = 1:sum(PR_methods.select)
    ...
    if n ==1
        GT_full = fnc_skeleton_analysis(PR_methods.roi.sk.('GT'),PR_methods.roi.bw, 1,Calibration);
    end
    % get some basic graph results for each method with varying threshold
    results_full = fnc_skeleton_analysis(PR_methods.sk.(method),PR_methods.roi.bw, T,Calibration);
    if iM == 1
        results.('GT') = GT_full;
...
function results = fnc_skeleton_analysis(sk_in,roi_in,T,Calibration)
results = table;
ROIArea = bwarea(roi_in)*Calibration^2;
for iT = 1:size(sk_in,3)
    sk = sk_in(:,:,iT);
    ...
    stats = regionprops(areas,'Area','Eccentricity','Perimeter');
    results.Threshold(iT) = T(iT);
    results.Edges(iT) = edges.NumObjects;
    results.Pixels(iT) = sum(sum(sk));
    results.Length(iT) = sum([stats.Perimeter])*Calibration/2;
    results.LengthDensity(iT) = results.Length(iT)/ROIArea;
    ...
    results.AreaMean(iT) = mean([stats.Area])*Calibration^2;
