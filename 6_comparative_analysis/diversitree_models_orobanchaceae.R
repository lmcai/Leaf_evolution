#This script is modified from Padfield et al., MBE 2024. Macroevolutionary Dynamics in Micro-organisms: Generalists Give Rise to Specialists Across Biomes in the Ubiquitous Bacterial Phylum Myxococcota
...
# read in tree
tree <- read.tree('Orobanchaceae_complete.tre')
#tree <- ladderize(tree, right = TRUE)
tree <- force.ultrametric(tree)

source('diversitree_helper_functions.R')

# setup for analyses ####

# reorder metadata to match tip labels of tree
d_meta <- tibble(tip_label = tree$tip.label) %>%
     left_join(., rename(d_leaf, tip_label = tip_label))
...
unique(d_meta$Leaf_type_code)

leaf_type <- setNames(d_meta$Leaf_type_code, d_meta$tip_label)
